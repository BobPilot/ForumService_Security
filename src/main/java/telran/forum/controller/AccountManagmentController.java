...
public class AccountManagmentController {
    ...
    public UserProfileRolesDto setRoleModerator(@PathVariable String id){
        return accountService.setModerator(id);
    }

	@PutMapping("/{id}")
	public UserProfileDto changePassword(@PathVariable String id,
								  @RequestHeader(value = "Authorization") String auth,
										 @RequestHeader(value = "Account-upd") String upd){
		return accountService.changePassword(id, auth, upd);
	}
